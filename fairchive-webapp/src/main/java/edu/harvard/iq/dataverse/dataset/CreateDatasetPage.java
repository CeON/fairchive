...
public class CreateDatasetPage implements Serializable {
    ...
    public String save() {
        ...
        if (!fieldValidationResults.isEmpty() || !constraintViolations.isEmpty()) {
            JsfHelper.addErrorMessage(getStringFromBundle.getStringFromBundle("dataset.message.validationError"),
            		getStringFromBundle.getStringFromBundle("dataset.message.validationErrorDetails"));
    ...
    public String finalizeSave() {
        ...
        Try.of(() -> saveDatasetProcess.getAddingFiles().get())
            .onFailure(ex -> handleErrorMessage(getStringFromBundle.getStringFromBundle("dataset.message.createSuccess.failedToSaveFiles"), ex))
    ...
    private void handleSuccessOrPartialSuccessMessages(int filesToSaveCount, AddFilesResult addFilesResult, boolean hasProvenanceErrors) {
        ...
        if (filesToSaveCount == addFilesResult.getSavedFilesCount()) {
            JsfHelper.addFlashSuccessMessage(getStringFromBundle.getStringFromBundle("dataset.message.createSuccess"));
        } else if (addFilesResult.getSavedFilesCount() == 0) {
            JsfHelper.addFlashWarningMessage(getStringFromBundle.getStringFromBundle("dataset.message.createSuccess.failedToSaveFiles"));
        } else {
            String partialSuccessMessage = getStringFromBundle.getStringFromBundle("dataset.message.createSuccess.partialSuccessSavingFiles",
        ...
        if (hasProvenanceErrors) {
            JsfHelper.addFlashErrorMessage(getStringFromBundle.getStringFromBundle("file.metadataTab.provenance.error"));
        }
    }
