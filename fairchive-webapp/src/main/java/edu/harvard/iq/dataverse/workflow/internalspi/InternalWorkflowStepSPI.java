...
public class InternalWorkflowStepSPI implements WorkflowStepSPI {
    ...
    @Inject
    public InternalWorkflowStepSPI(final WorkflowStepRegistry stepRegistry, 
    		final DatasetVersionServiceBean datasetVersions,
    ...
    @PostConstruct
    public void init() {
        this.stepRegistry.register(INTERNAL_PROVIDER_ID, this);
    }

    // -------------------- LOGIC --------------------

    @Override
    public WorkflowStep getStep(final String stepType, 
    		final WorkflowStepParams stepParameters) {
        switch (stepType) {
            ...
            case "http/sr":
                return new HttpSendReceiveClientStep(stepParameters, 
                		this.datasetVersions, this.citationFactory);
